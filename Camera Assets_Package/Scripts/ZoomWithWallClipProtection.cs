...
public class ZoomWithWallClipProtection : MonoBehaviour
{
    ...
    public float maxDistance = 1.5f;       // the closest distance the camera can be from the target
    ...
    private float m_FixDistance;             // the original distance to the camera before any modification are made
    ...
    private void Start()
    {
        ...
        m_CurrentDist = m_Cam.localPosition.magnitude;
        m_FixDistance = m_CurrentDist;
    ...
    private void LateUpdate()
    {
        ...
        float targetDist = m_FixDistance;
        ...
        if (initialIntersect)
        {
            ...
            m_Hits = Physics.RaycastAll(m_Ray, m_FixDistance - sphereCastRadius);
        }
        else
        {
            // if there was no collision do a sphere cast to see if there were any other collisions
            m_Hits = Physics.SphereCastAll(m_Ray, sphereCastRadius, m_FixDistance + sphereCastRadius);
        }
        ...
        m_FixDistance -= Input.GetAxis("Mouse ScrollWheel") * zoomSpeed;
        m_CurrentDist = Mathf.SmoothDamp(m_CurrentDist, targetDist, ref m_MoveVelocity,
                                       m_CurrentDist > targetDist ? clipMoveTime : returnTime);
        m_CurrentDist = Mathf.Clamp(m_CurrentDist, minDistance, maxDistance);
